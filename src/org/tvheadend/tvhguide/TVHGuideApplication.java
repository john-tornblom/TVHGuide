...
public class TVHGuideApplication extends Application {
	...
	public void broadcastError(final String error) {
		...
		handler.post(new Runnable() {
			...
			public void run() {
				...
				try {
					Toast toast = Toast.makeText(TVHGuideApplication.this,
	...
	public static SparseArray<String> getContentTypes(Resources resources) {
		SparseArray<String> ret = new SparseArray<String>();

		String[] s = resources.getStringArray(R.array.pr_content_type0);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x00 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type1);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x10 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type2);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x20 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type3);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x30 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type4);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x40 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type5);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x50 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type6);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x60 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type7);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x70 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type8);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x80 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type9);
		for (int i = 0; i < s.length; i++) {
			ret.append(0x90 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type10);
		for (int i = 0; i < s.length; i++) {
			ret.append(0xa0 + i, s[i]);
		}

		s = resources.getStringArray(R.array.pr_content_type11);
		for (int i = 0; i < s.length; i++) {
