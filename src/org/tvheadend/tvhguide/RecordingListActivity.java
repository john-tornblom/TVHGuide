...
public class RecordingListActivity extends ListActivity implements HTSListener {
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		if (rec.isRecording() || rec.isScheduled()) {
			...
			item = menu.add(ContextMenu.NONE, R.string.ch_play,
	...
	private class ViewWarpper {
		...
		public void repaint(Recording rec) {
			...
			} else if (rec.start.getTime() < System.currentTimeMillis() + 1000
					* 60 * 60 * 24 * 2
					&& rec.start.getTime() > System.currentTimeMillis() - 1000
							* 60 * 60 * 24 * 2) {
				date.setText(DateUtils.getRelativeTimeSpanString(
						rec.start.getTime(), System.currentTimeMillis(),
			...
			time.setText(DateFormat.getTimeFormat(time.getContext()).format(
					rec.start)
					+ " - "
					+ DateFormat.getTimeFormat(time.getContext()).format(
							rec.stop));
			time.invalidate();
		}
	}
	...
	class RecordingListAdapter extends ArrayAdapter<Recording> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (row == null) {
				...
				row = inflater.inflate(R.layout.recording_list_widget, null,
						false);
